...
describe("template-base", function () {
  ...
  describe("default template options", function () {
    ...
    var Template;
    beforeEach(function () {
      Template = SandboxedModule.require(
        "../../lib/template-base"
      ).TemplateBase;
  ...
  describe("generate files based on template", function () {
    ...
    beforeEach(function () {
      ...
      testTemplate = TestTemplate.newWithDirectory(
        "/minit_home/templates/testTemplate"
      );
    ...
    it("interpolates strings correctly", function () {
      var result = testTemplate.applyTransform("before {{replace}} after", {
        replace: "a/b/c",
      });
